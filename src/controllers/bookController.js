...
const updateBookById = async (req, res) => {
  try {
    ...
    //duplicate title & ISBN
    if (existsBook.title === title) return res.status(400).send({ status: false, message: ` '${title}' this title already exists.` });
    if (existsBook.ISBN === ISBN) return res.status(400).send({ status: false, message: ` '${ISBN}' this ISBN already exists.` });
